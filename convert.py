...
def main():
	...
	json_from_dir(directory)

def json_from_dir(directory):
...
def tag_items_from_file_items(file_items):
	...
	for file_item in file_items:
		for tag in file_item['content']['appData']['de.fxnn.standardnotes']['tags']:
			if tag in tags:
				tag_item = tags[tag]
			else:
				tag_item = {
			...
			create_reference(tag_item, file_item)
			create_reference(file_item, tag_item)
	return result

def create_reference(item1, item2):
